...
const SYSTEM_PROMPT = `You are the AEGIS Intelligence Analyst — a senior intelligence analyst embedded inside the AEGIS global intelligence surface.
...
const BRIEFING_PROMPT = `Generate an AEGIS daily intelligence briefing.
...
export function generateLocalAnalysis(context: IntelligenceContext, userQuery: string): string {
  ...
  lines.push('');
  lines.push('## MODE');
  lines.push('AEGIS Local Analyst (zero-cost fallback — no external model spend required).');
  return lines.join('\n');
}
...
export function generateLocalBriefing(context: IntelligenceContext): string {
  ...
  return [
    ...
    '## I. EXECUTIVE SUMMARY',
    topThreat
      ? `AEGIS is tracking a ${topThreat.severity.toLowerCase()} threat-led operating picture centered on ${topThreat.region}.`
      : topEq
        ? `AEGIS is tracking a seismic-led operating picture centered on ${topEq.location}.`
        : topNews
          ? `AEGIS is tracking a news-led risk picture with the strongest signal coming from ${topNews.source}.`
          : 'AEGIS is in broad monitoring posture with limited high-confidence escalation signals at this time.',
    '',
    '## II. PRIORITY DEVELOPMENTS',
    ...
    `**${confidence}**`,
    '',
    '_Generated by AEGIS Local Analyst (zero-cost mode)._'
  ].join('\n');
}
...
export function generateLocalFusionDossier(context: IntelligenceContext): LocalFusionDossier {
  ...
  const bluf = topThreat
    ? `AEGIS assesses the current operating picture as ${riskLevel.toLowerCase()}, driven primarily by ${topThreat.severity.toLowerCase()} threat activity centered on ${topThreat.region}.`
    : topEq
      ? `AEGIS assesses the current operating picture as ${riskLevel.toLowerCase()}, driven primarily by seismic activity near ${topEq.location}.`
      : topNews
        ? `AEGIS assesses the current operating picture as ${riskLevel.toLowerCase()}, with the strongest live signal coming from high-risk reporting via ${topNews.source}.`
        : 'AEGIS assesses the current operating picture as low-risk with limited high-confidence escalation indicators in current telemetry.';
