...
const NUCLEAR_FACILITIES: Facility[] = [
  ...
  { id: 'nuc-ru-kola', name: 'Kola NPP', city: 'Polyarnyye Zori', country: 'Russia', lat: 67.4642, lng: 32.475, status: 'Operational', reactors: 4, capacityMW: 1760, owner: 'Rosenergoatom' },
  ...
  { id: 'nuc-ru-beloyarsk', name: 'Beloyarsk NPP (BN-800)', city: 'Zarechny', country: 'Russia', lat: 56.8419, lng: 60.725, status: 'Operational', reactors: 2, capacityMW: 1400, owner: 'Rosenergoatom' },
  ...
  { id: 'nuc-us-south-texas', name: 'South Texas Project', city: 'Bay City', country: 'US', lat: 28.795, lng: -96.0481, status: 'Operational', reactors: 2, capacityMW: 2560, owner: 'STP Nuclear' },
  ...
  { id: 'nuc-jp-takahama', name: 'Takahama NPP', city: 'Takahama', country: 'Japan', lat: 35.5217, lng: 135.495, status: 'Partially Operational', reactors: 4, capacityMW: 3392, owner: 'KEPCO' },
  ...
  { id: 'nuc-in-tarapur', name: 'Tarapur NPP', city: 'Tarapur', country: 'India', lat: 19.8306, lng: 72.655, status: 'Operational', reactors: 4, capacityMW: 1400, owner: 'NPCIL' },
...
export async function GET() {
  ...
  try {
    ...
    if (res.ok) {
      const eqData = await res.json() as EarthquakeFeed;
      const earthquakes = eqData.features || [];

      dynamicFacilities = NUCLEAR_FACILITIES.map((facility) => {
        const nearbyQuakes = earthquakes.filter((earthquake) => {
          const coordinates = earthquake.geometry?.coordinates;
          if (!coordinates) return false;
          const [eqLng, eqLat] = coordinates;
          return getDistanceKm(facility.lat, facility.lng, eqLat, eqLng) < 150;
        });

        if (nearbyQuakes.length === 0) return facility;

        const maxMag = Math.max(...nearbyQuakes.map((earthquake) => earthquake.properties?.mag || 0));
        return {
          ...facility,
